...
void loop()
{
  ...
  right_sonar = sonar[Right].ping_cm()*15;       // 우측 센서 츨측정
  ...
  if(maze_status == 4)
  {
    ...
    motor_A_contro1(LOW,0);  //오른쪽은 전진
    motor_B_contro1(HIGH,0);  //왼쪽은 정지
    delay(500);                //일정한 시간 동안 정지
    
     //180도 회전을 한다
    Serial.println("Rotate CCW");
    motor_A_contro1(LOW,118);  //오른쪽은 전진
    motor_B_contro1(LOW,120);  //왼쪽은 전진
    delay(1000);
  ...
  else if(maze_status == 1)
  {
    ...
    motor_A_contro1(LOW,100);  //오른쪽은 전진
    motor_B_contro1(HIGH,100);  //왼쪽은 전진
  }
  else{
    Serial.println("FOWARD");
    motor_A_contro1(LOW,100);  //오른쪽은 전진
